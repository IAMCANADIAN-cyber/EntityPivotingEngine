...
int ares_dup(ares_channel *dest, ares_channel src)
{
  ...
  *dest = NULL; /* in case of failure return NULL explicitly */
  ...
  /* Full name server cloning required if there is a non-IPv4, or non-default po
rt, nameserver */
                                ...
                                const ULONG interfaceMetric)
{
   ...
   *
   * See
   * http://msdn.microsoft.com/en-us/library/windows/desktop/aa814494(v=vs.85).a
spx
                         ...
                         const char *filech)
{
  ...
  while (*p)
    {
      if ((*p == *bindch || *p == *altbindch || *p == *filech) && l < lookups +
2) {
