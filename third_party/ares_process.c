...
static ares_ssize_t socket_writev(ares_channel channel, ares_socket_t s, const s
truct iovec * vec, int len)
{
...
static ares_ssize_t socket_write(ares_channel channel, ares_socket_t s, const vo
id * data, size_t len)
{
  if (channel->sock_funcs)
    {
  ...
  for (i = 0; i < channel->nservers; i++)
    {
      ...
      if (vec)
      ...
      else
        {
          ...
          scount = socket_write(channel, server->tcp_socket, sendreq->data, send
req->len);
          if (scount < 0)
            {
              if (!try_again(SOCKERRNO))
                           ...
                           struct timeval *now)
{
  ...
  packetsz = PACKETSZ;
  /* If we use EDNS and server answers with FORMERR without an OPT RR, the proto
col
                      ...
                      struct timeval *now)
{
  ...
  if (query->using_tcp)
  ...
  else
    {
      ...
      if (socket_write(channel, server->udp_socket, query->qbuf, query->qlen) ==
 -1)
        {
